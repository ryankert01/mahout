...
The best place to start is by looking into our [GitHub Issues](https://github.com/apache/mahout/issues) and
...
for making changes to the code.  Changes, especially from non-committers, need to be evolutionary
...
Hang out on the dev@ mailing list. That's where we discuss
...
are available through the Apache Software Foundation.

<a name="HowToContribute-ContributingCode(Features,BigFixes,Tests,etc...)"></a>
## Contributing Code (Features, Bug Fixes, Tests, etc...)

This section identifies the ''optimal'' steps community members can take to
submit changes or additions to the Mahout code base.	This can be new
...
don't have the time or resources to do everything outlined below
should not be discouraged from submitting their ideas "as is" per "Yonik
Seeley's (Solr committer) Law of Patches":

*A half-baked patch in a GitHub issue, with no documentation, no tests and no backwards compatibility is better than no patch at all.*
...
First of all, you need to get the Mahout source code. Mahout's codebase is hosted on [GitHub](https://github.com/apache/mahout). 
The first step to making a contribution is to fork Mahout's master branch to your GitHub repository.
...
(note: you have to subscribe before you can post), or file an issue in [GitHub Issues](https://github.com/apache/mahout/issues).
...
Run `pytest`, if you see all tests passing, all is ok, but if you see failures,
please carefully read the error messages and check your code.
...
git to figure out what's changed and by whom.
...
for improvement (more tests, better documentation, etc...) then make the changes on your GitHub branch and add a comment about them. If a lot of people review a Pull Request and give it a
