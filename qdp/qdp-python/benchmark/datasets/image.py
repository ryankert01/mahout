...
class FullMNISTDataset(BenchmarkDataset):
    ...
    def __init__(
        ...
        self._class_0 = (
            [class_0]
            if isinstance(class_0, int)
            else (list(class_0) if class_0 else None)
        )
        self._class_1 = (
            [class_1]
            if isinstance(class_1, int)
            else (list(class_1) if class_1 else None)
        )
        self._seed = seed
        self._X: Optional[np.ndarray] = None
        self._y: Optional[np.ndarray] = None
    ...
    @property
    def name(self) -> str:
        if self._is_binary and self._class_0 is not None and self._class_1 is not None:
            c0 = ",".join(map(str, self._class_0))
            c1 = ",".join(map(str, self._class_1))
