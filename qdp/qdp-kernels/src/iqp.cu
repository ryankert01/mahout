...
// Compute the normalized IQP amplitude for basis state z:
//   amplitude[z] = (1/2^n) * sum_x exp(i*theta(x)) * (-1)^popcount(x AND z)
__device__ cuDoubleComplex compute_iqp_amplitude(
    const double* __restrict__ data,
    size_t z,
    size_t state_len,
    unsigned int num_qubits,
    ...
    return make_cuDoubleComplex(real_sum * norm, imag_sum * norm);
}
...
__global__ void iqp_encode_kernel_naive(
    ...
    state[z] = compute_iqp_amplitude(data, z, state_len, num_qubits, enable_zz);
}
...
__global__ void iqp_encode_batch_kernel_naive(
    ...
    for (size_t global_idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        state_batch[global_idx] = compute_iqp_amplitude(data, z, state_len, num_qubits, enable_zz);
    }
}
